...
```
Определите переменную окружения `ROLLBAR_TOKEN` системы логирования [Rollbar](https://rollbar.com/). Когда речь зайдёт о SDK, выбирайте Django и получите инструкции по интеграции, в ней будет указан персональный токен.
```sh
ROLLBAR_TOKEN=eb1c26e92fd745b484556f0e3325f080
```
Вы можете определить переменную окружения `ENVIRONMENT`, чтобы система логирования могла различать среды исполнения. По умолчанию будет присвоено значение `development`.
```sh
